...
function cargarTodo() {
    ...
    pedidos = leer(STORAGE_PEDIDOS);
    clientes = leer(STORAGE_CLIENTES);
    proveedores = leer(STORAGE_PROVEEDORES);
    recordatoriosGenerales = leer(STORAGE_RECORDATORIOS_GENERALES);
}
...
// y solo existía "próxima acción" en vez de una lista de recordatorios).
function migrarDesdeV1SiHaceFalta() {
    if (localStorage.getItem(STORAGE_PEDIDOS)) return; // ya se usó la v2 en este navegador
    const v1raw = localStorage.getItem(STORAGE_PEDIDOS_V1);
    if (!v1raw) return; // instalación nueva, no había nada antes

    try {
        const pedidosV1 = JSON.parse(v1raw);
        pedidosV1.forEach(p => {
            let clienteId = null;
            if (p.cliente && p.cliente.nombre) {
                let cliente = clientes.find(c => c.nombre.toLowerCase() === p.cliente.nombre.toLowerCase());
                if (!cliente) {
                    cliente = {
                        id: generarId(), nombre: p.cliente.nombre, telefono: p.cliente.telefono || '',
                        email: p.cliente.email || '', direccion: p.cliente.direccion || '', origen: p.cliente.origen || '',
            ...
            const recordatorios = [];
            if (p.proximaAccion && p.proximaAccion.texto) {
                recordatorios.push({ fecha: p.proximaAccion.fecha || '', texto: p.proximaAccion.texto, hecho: false });
            }
            const { cliente, proximaAccion, ...resto } = p;
            pedidos.push({ ...resto, clienteId, recordatorios, fotos: p.fotos || [] });
        });
        guardarTodo();
        console.log(`Migración desde CRM v1 completada: ${pedidosV1.length} pedido(s).`);
    } catch (e) {
        console.error('No se pudo migrar datos de la versión anterior del CRM:', e);
    }
}
...
// UTILIDADES
// ==========================================================================
function generarId() { return 'id_' + Date.now() + '_' + Math.floor(Math.random() * 1000); }
...
window.marcarRecordatorioHecho = function(origen, origenId, idx, hecho) {
    let arr;
    ...
    if (arr && arr[idx]) { arr[idx].hecho = hecho; guardarTodo(); renderTodo(); }
};
...
function onDropColumna(evt, etapaDestino) {
    evt.preventDefault();
...
window.guardarRecordatorioGeneral = function() {
    ...
    recordatoriosGenerales.push({ fecha: fecha || new Date().toISOString().substring(0, 10), texto, hecho: false, clienteId, proveedorId });
    guardarTodo();
...
window.borrarProveedor = function() {
...
window.marcarComoPerdido = function() {
...
function cargarDatosDeEjemplo() {
    ...
    recordatoriosGenerales = [{ fecha: dias(4), texto: 'Renovar stock de espuma D28 para tapicería', hecho: false }];
...
window.importarBackup = function(inputEl) {
    const file = inputEl.files[0];
    if (!file) return;
    if (!confirm('Importar un backup va a REEMPLAZAR todos los datos actuales del CRM en este navegador. ¿Continuar?')) { inputEl.value = ''; return; }
    const reader = new FileReader();
    reader.onload = (e) => {
        try {
            ...
            recordatoriosGenerales = data.recordatoriosGenerales || [];
            if (!guardarTodo()) return;
            renderTodo();
